...
do -- Data Setup
	local cryIds = { 137839903222495, 90137205897663, 99451190033469, 131266660537547, 12976016124,--// Cries 1-5
		131073394494295, 80691630531507, 104972413162665, 113864184824720, 91805306702945,--// Cries 6-10
		103860266396572, 110865473425087, 78220978141715, 78324392590042, 12976143735,--// Cries 11-15
		12976147743, 135176534966371, 13592572217 }--// Cries 16 & 18
...
--Colo Banlist
local Banlist = require (storage.Data.ColoBanlist.OU)
for _, d in pairs(Banlist) do
	Banlist [d.name] = d
end

local UU = require (storage.Data.ColoBanlist.UU)
for _, d in pairs(UU) do
	UU [d.name] = d
end

local Ubers = require (storage.Data.ColoBanlist.Ubers)
for _, d in pairs(Ubers) do
	Ubers [d.name] = d
end

local UUbers = require (storage.Data.ColoBanlist.UUbers)
for _, d in pairs(UUbers) do
	UUbers [d.name] = d
end
