...
Gold   Crown 15883232524 (offset lower)
Silver Crown 15883238555 (centered)
Bronze Crown 15883242718 (high but small so centered)
...
local elo = Utilities.class({
	...
	local self = {
		metaStore = _f.safelyGetDataStore('EloMeta'..name),
		rankStore = _f.SirJayService:GetOrderedBase("EloRanks_"..name),
		rankCache = {},
		failedToLoad = {},
...
function elo:adjustRanks(id1, id2, winner) -- "winner" is expected to match one of the first two arguments, or is draw
	...
	local player1 = game.Players:GetPlayerByUserId(tonumber(rid1))
	local player2 = game.Players:GetPlayerByUserId(tonumber(rid2))

	local format = player1 and _f.PlayerDataService[player1]:getFormat() or "AG"
	...
	if format == "AG" then
		k1 = 0
		k2 = 0
	elseif format == "Ubers" then
		k1 = 0
		k2 = 0
	elseif format == "UUbers" then
		k1 = 0
		k2 = 0
	elseif format == "UU" then
		k1 = 0
		k2 = 0
	end
