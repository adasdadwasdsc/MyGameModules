...
local banlist = SirJayService:GetSirJay('Banned')
...
local function playerNotificationListUpdated(player, list)
	stores:GetSirJay('Notification', tostring(player.UserId)):UpdateAsync('Inbox', function(list)
...
local function subscribePlayer(player)
	...
	playerSubscriptions[player.Name] = stores:GetSirJay('Notification', tostring(player.UserId)):OnUpdate('Inbox', function(list)
...
local function sendNotification(recipientId, n)
	...
	if recipientId then
		global = false
		recipientStore = stores:GetSirJay('Notification', tostring(recipientId))
	end
	n.ts = os.time()
	n.id = uid()
